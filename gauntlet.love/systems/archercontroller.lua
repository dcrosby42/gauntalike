...
local archerControllerSystem = defineUpdateSystem({'archer','controller'}, function(e,estore,input,res)
  local c = e.controller

  -- Set velocity based on left stick
  if e.force then
    e.force.fx = c.leftx * e.archer.speed
    e.force.fy = c.lefty * e.archer.speed
  else
    e.vel.dx = c.leftx * e.archer.speed
    e.vel.dy = c.lefty * e.archer.speed
  end
